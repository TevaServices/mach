...
- Regular `mach audit` reviews; revoke machines on decommission with
  ...
  already redacted).
- The SQLite file is `0600` (the control plane tightens the database and its
  WAL sidecar on every open, so a loose volume is corrected) — but the *host*
  is still the boundary: anyone who can read that file reads the audit trail
  and the API-key hashes. Keep it off a shared mount.
